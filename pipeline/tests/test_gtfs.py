...
from tirana_pipeline.assets.gtfs import gtfs_stops, gtfs_routes
...
def test_gtfs_stops_returns_geodataframe():
...
def test_gtfs_stops_geometry_points():
    raw = _make_gtfs_zip(STOPS_CSV, ROUTES_CSV)
    gdf = gtfs_stops(MockContext(), raw)
    assert all(gdf.geometry.geom_type == "Point")


def test_gtfs_routes_returns_dataframe():
    raw = _make_gtfs_zip(STOPS_CSV, ROUTES_CSV)
...
def test_gtfs_stops_missing_columns_raises():
